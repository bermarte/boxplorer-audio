...
#include "includeFMOD/fmod_errors.h"
...
{
    // initialise fmod
	if( FSOUND_Init(44000,32,0) == FALSE )
	{
    ...
    http://kerrenortlepp.wordpress.com/2013/05/20/getting-file-path-using-the-open-file-dialog-in-c/
	*/
	/**< Open file dialog */
    HWND hwnd; /**< Hardware handler? */
    HANDLE hf; /**< File handler */
    OPENFILENAME ofn_test = {0};
    string tempString; /**< I dunno why this is here, what is it used for :D??? */

    ZeroMemory(&ofn_test, sizeof(ofn_test));
    ofn_test.lStructSize = sizeof(ofn_test);

    char filename_cstring[2048] = {0}; /**< Create a buffer for the file name, technically I should be able to use this for the filename? */
    ofn_test.lpstrFile = filename_cstring;
    ofn_test.nMaxFile = 2048;
    ofn_test.lpstrTitle = tempString.c_str();
    GetOpenFileName(&ofn_test); /**< This is a boolean so you can check whether it succeeded or failed */

    char* filename = ofn_test.lpstrFile; /**< Save the file path */

    /**< Adapted from http://latedev.wordpress.com/2011/10/06/simple-windows-dialogs-from-c-part-1/ */


	g_mp3_stream = FSOUND_Stream_Open( filename , FSOUND_2D , 0 , 0 );
...
#define PROCESS_CONFIG_PARAMS \
  ...
  PROCESS(float, specL, "specL", true) \
  ...
  PROCESS(float, fft_out, "fft_out", true)
...
#define NSUBFRAMES 100  // # splined subframes between keyframes.//oppenhaimer :-)
