...
#define Offset vec3(par[4].y,par[4].x,par[5].y)+fft_out/100.
...
#define DEoffset par[0].x*/
	...
	//rmin=min(1.,r2);
	return mix(sqrt(rmin),(0.5+0.5*sin(col.z*vec3(par[6].y,par[6].x,par[7].y))),par[7].x);//vec3(sqrt(rmin));//*col.xyz/(iters+1.);
}
