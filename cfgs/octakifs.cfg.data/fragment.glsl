...
uniform float incr;//berny
uniform float time;
//uniform Channel channel;
uniform float sound;
uniform float spec;
uniform float ssx;
uniform float fft_out;
#define fft_incr par[8].y// {min=-113.0 max=113 step=.001}
...
//#define Angle time
//#define Angle 0.
#define Angle2 (fft_out+fft_incr)/100.0
//#define Angle2 time/10.
#define Angle par[3].y// {min=-6.0 max=6 step=.001}
...
//float Angle = par[3].x;
//channel->getSpectrum( 234, SPECLEN, 0, FMOD_DSP_FFT_WINDOW_BLACKMAN );

float csat = cos(Angle);//specL
float ssat = sin(Angle);
float usat = 1.0-cos(Angle);
//

float csat2 = cos(Angle2);//specL
float ssat2 = sin(Angle2);
float usat2 = 1.0-cos(Angle2);
vec3 z0 = normalize(vec3(par[8].y, par[3].y, par[3].x));
...
mat3 RotationMatrix2 = mat3( z0.x*z0.x*usat2 + csat2,      z0.x*z0.y*usat2 + z0.z*ssat2, z0.x*z0.z*usat2 - z0.y*ssat2,
                            z0.y*z0.x*usat2 - z0.z*ssat2, z0.y*z0.y*usat2 + csat2,      z0.y*z0.z*usat2 + z0.x*ssat2,
			    z0.z*z0.x*usat2 + z0.y*ssat2, z0.z*z0.y*usat2 - z0.x*ssat2, z0.z*z0.z*usat2 + csat2
		      );
